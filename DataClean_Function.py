...
def clean(df):
    ...
    for i, row in df.iterrows():
        ...
        df.at[i, 'Patient ID#'] = patient_id  # replace ID line item
        ...
        if not re.match(r'^(([1-9]|[1][0-2])\/([1-9]|[1-2][0-9]|[3][0-1])\/(\d\d))$',
                        request_date) and request_date != "Missing":
        ...
        df.at[i, 'Request Status'] = request_status  # replace line item
        ...
        df.at[i, 'Payment Submitted?'] = payment_status  # replace line item

        reason = row['Reason - Pending/No']  # make sure reason is text
        if type(reason) != str:
            reason = ""
        reason.strip()
        df.at[i, 'Reason - Pending/No'] = reason
        ...
        df.at[i, 'Pt City'] = city  # replace line item
        ...
        df.at[i, 'Pt Zip'] = zipcode  # replace zip code line item
        ...
        df.at[i, 'Language'] = language  # replace line item
        ...
        df.at[i, 'Marital Status'] = maritial_status  # replace line item
        ...
        df.at[i, 'Gender'] = gender  # replace line item
        ...
        if race[0:3] == "Whi":  # fix inconsistency
            race = "White"
        df.at[i, 'Race'] = race  # replace line item
        ...
        df.at[i, 'Hispanic/Latino'] = hispanic  # replace line item
        ...
        if sexuality[0:2] == "St" or sexuality[0:3] == "Het":
            ...
            sexuality = "Missing"
        df.at[i, 'Sexual Orientation'] = sexuality  # replace line item
        ...
        if insurance == '' or insurance == "Unknown":  # make missing data consistent
            ...
            insurance = "Uninsured"
        df.at[i, 'Insurance Type'] = insurance  # replace line item
        ...
        df.at[i, 'Referral Source'] = referral  # replace line item
        ...
        df.at[i, 'Referred By:'] = referral2  # replace line item
        ...
        if method == "?" or method == "":  # make missing data consistent
            method = "Missing"
        df.at[i, 'Payment Method'] = method  # replace line item
        ...
        df.at[i, 'Payable to:'] = payable_to  # replace line item
        ...
        if letter == "n/a" or letter == "na":  # make N/A consistent
            ...
            letter = "Yes"
        letter = letter.title()
        df.at[i, 'Patient Letter Notified? (Directly/Indirectly through rep)'] = letter  # replace line item
        ...
        df.at[i, 'Application Signed?'] = signed  # replace line item
